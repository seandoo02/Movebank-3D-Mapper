...
ggplot(data = DATA, aes(x = location.long, y = location.lat)) +  
  geom_point(color = "blue") +  # Use geom_point() instead of points()
...
SUB <- DATA[1:length(DATA$timestamp),]
# plot subset
ctmm::plot(SUB)
print(sub)

SEQ <- seq(from=SUB$t[1],to=SUB$t[length(DATA$timestamp)],by=5 %#% 'min')
...
plot(list(SIM,SIM2,SUB),col=c('blue','orange','black','red'),type=c('l','l','p'))
...
# Convert to sf object with UTM Zone 46N CRS
utm_crs <- crs(landsat_rgb_cropped,asText=TRUE)
data_sf <- st_as_sf(DATA, coords = c("x", "y"), crs = utm_crs)

# Transform to WGS84 (lat/lon)
data_sf <- st_transform(data_sf, crs = 4326)

# Extract the lat/lon values
latlon <- st_coordinates(data_sf)  # Extracts as matrix (lon, lat)
```
...
please_work=project(SIM[2,3],from="WGS84" , to="WGS84")
please_work
```
```{r}
...
```{r}
utm_zone <- floor((mean(DATA$longitude, na.rm = TRUE) + 180) / 6) + 1
print(utm_zone)
```
