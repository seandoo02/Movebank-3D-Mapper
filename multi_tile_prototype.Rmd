...
# make_bbox
```{r}
# Creates a bounding box for a movment bank formated dataframe
...
crop_rasters=function(e, landsat_rbg, elevation_utm){
 ...
 # print("landsat is cropped")
  elevation_cropped = raster::crop(elevation_utm, e)
 # print("elevation is cropped")
  ...
  #plot the raster brick
  #landsat_rgb_raster=raster::brick(landsat_rgb_array)
  #raster::plotRGB(landsat_rgb_raster, r = 1, g =2 , b = 3, stretch = "lin")
  #raster::plotRGB(landsat_rgb_raster)
  #rayimage::plot_image(landsat_rgb_array)
  # Set interpolate = TRUE for smoothing
...
# make_color_vector
```{r}
#the render_point method in the rayshader package needs a color vetor where each entrie coresponds to a point that will be ploted
...
```{r}
# converts the data frame lat long to the same CRS that is being ploted with the rayshader plot 3d method
convert_location_data_to_utm=function(df, elevation_cropped){
  ...
  utm_lats=c() #ik this is a terible variable name 
  utm_longs=c()
...
#df=read.csv('movebank_data/Mountain caribou in British Columbia-gps.csv')
...
```{r}
# paths_rows=get_path_row(df_bbox)
# print(paths_rows)
```
```{r}
# Step 1: Define bounding box coordinates
  ...
  # Initialize an empty list to store path-row pairs
...
for(i in 1:length(paths_rows)){
  ...
  #print(indexes_of_intrest)
  min_cloud_index=find_min_cloud(landsat_obj,indexes_of_intrest)
  min_cloud_of_intrest=append(min_cloud_of_intrest,min_cloud_index)
  #print("___________")
}
print(min_cloud_of_intrest)
```


```{r}
landsat_tiles=list()
for(i in min_cloud_of_intrest){
...
#plot cropped rgb landsat data
rayshader::plot_map(landsat_rgb_array)
```
...
```{r}
rayshader::plot_map(landsat_rgb_contrast)
```

```{r}
landsat_rgb_cropped=raster::brick(landsat_rgb_contrast)
extent(landsat_rgb_cropped)=e
print(dim(landsat_rgb_cropped))
...
```{r}
crs(landsat_rgb_cropped)=crs(merged_rgb, asText=TRUE)
```

```{r}
crs(landsat_rgb_cropped,asText=TRUE)
```
...
```{r}
for(i in index_list){
  hue=head(sequential_hcl(n=10, sequential_hues[test_index[i]]),-3)
  raster::plot(1-akde_list[[i]]$CDF$layer, alpha=.5, col = hue ,legend=FALSE)
}
```

```{r}
temp=matrix(data=1, nrow=dim(landsat_rgb_cropped)[[1]],ncol=dim(landsat_rgb_cropped)[[2]])
r=raster(temp)
r3=raster::stack(r,r,r)
extent(r3)=extent(landsat_rgb_cropped)
crs(r3)=crs(landsat_rgb_cropped)
plotRGB(r3,scale=max(1))
individual_CI= akde_list_sorted[[1]]$CI
...
library(tiff)
 ...
 #godfather_CDF=godfather_akde$CDF
 #godfather_CDF_utm=raster::projectRaster(godfather_CDF,landsat_rgb_cropped,method='bilinear')
 #crs(godfather_CDF_utm,asText=TRUE)
 #CDF_resampled <- terra::resample(godfather_CDF_utm, landsat_rgb_cropped)
#base_raster=landsat_rgb_cropped
base_raster=r3
color_index=sample(1:110,length(akde_list),replace=FALSE)
j=1
...
for(i in index_list){
  tiff("landsat_with_overlay.tiff",
     width = ncol(base_raster),   # Width of the image in pixels
     height = nrow(base_raster), # Height of the image in pixels
     compression = "none") 
  
  # plotRGB(base_raster, r = 1, g = 2, b = 3,
  #         scale = maxValue(landsat_rgb_cropped)
  #         )
  plotRGB(base_raster,scale=max(1))

  #raster::plotRGB(landsat_rgb_contrast)
  #raster::plotRGB(landsat_rgb_cropped, r = 1, g =2 , b = 3, stretch = "lin")
  #rayshader::plot_map(landsat_rgb_contrast)
  # grid.newpage()  # Start a new page for plotting
  # grid.raster(landsat_rgb_contrast, interpolate = FALSE)
  # Plot the rescaled image

  #make color vecotor and AKDE colors
  #color_vector=make_color_vector(df)
  ...
  #set to correct CRS
  #crs(individual_CDF_utm,asText=TRUE)
  CDF_resampled=individual_CDF_utm
  #CDF_resampled <- terra::resample(individual_CDF_utm, landsat_rgb_cropped)
  CDF_resampled$layer[CDF_resampled$layer < .01] <- NA
  #Convert AKDE raster to a 3D array
  CDF_array <- as.array(CDF_resampled)
  color=hcl.pals(type=NULL)[color_index[i]]
  #raster::plot(1-CDF_resampled$layer, add = TRUE, alpha = .5, col = hcl.colors(20, color),legend=FALSE)
  hue=head(sequential_hcl(n=20, sequential_hues[test_index[j]]),-3)
  raster::plot(1-CDF_resampled$layer, add = TRUE, alpha = .5,col=hue,legend=FALSE)
  j=j+1
  dev.off()
  ...
  rm(base_raster)  # Remove the variable from the environment
  ...
  # print("3")
  # # Stack the RGB layers into a RasterBrick
  # blended_raster<- raster::brick(red_channel, green_channel, blue_channel)
  
  
  # Convert the matrix to a raster
  #blended_raster <- as.raster(png_image)
  dim(base_raster)
  #blended_raster= terra::rast(blended_raster)
  
  # Assign extent to match your original raster (e.g., landsat_rbg)
  #terra::ext(blended_raster) <- terra::ext(landsat_rbg)
  extent(base_raster)=extent(landsat_rgb_cropped)
  
  # Assign CRS (coordinate reference system) to match the original raster
  crs(base_raster) <- crs(landsat_rgb_cropped,asText=TRUE)
  # Rescale each band
  # base_raster <- stack(
  #   rescale_to_255(base_raster[[1]]),  # Red
  #   rescale_to_255(base_raster[[2]]),  # Green
  #   rescale_to_255(base_raster[[3]])   # Blue
  # )
...
#hillshade_rgb_array = aperm(hillshade_rgb_array, c(2,1,3))
```
...
color_vector=make_color_vector_AKDE(df_individual, n=10, sequential_hues = sequential_hues, index=test_index, inividuals )
```

```{r}
utm_locations=convert_location_data_to_utm(df_individual, elevation_cropped)
#utm_locations=convert_location_data_to_utm(df, elevation_cropped)
